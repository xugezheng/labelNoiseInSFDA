...
class ImageList(datasets.VisionDataset):
    ...
    def __init__(self, root: str, classes: List[str], data_list_file: str, r: float,
                 transform: Optional[Callable] = None, target_transform: Optional[Callable] = None):
        super().__init__(root, transform=transform, target_transform=target_transform)
        self.classes = classes
        self.class_to_idx = {cls: idx
                             for idx, cls in enumerate(self.classes)}
        self.samples, self.targets = self.parse_data_file(data_list_file)
    ...
    def __getitem__(self, index: int) -> Tuple[Any, int, int]:
        ...
        """
        (path, ind), target = self.samples[index], self.targets[index]
    ...
    def parse_data_file(self, file_name: str) -> Tuple[ List[Tuple[str, int]], List[int]]:
        ...
        with open(file_name, "r") as f:
            ...
            for line in f.readlines():
                ...
                target = int(target)
                if 'domainnet' in file_name:
                    cls_name = split_line[0].split('/')[1]
