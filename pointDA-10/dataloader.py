...
class Modelnet40_data(data.Dataset):
    def __init__(self, pc_root, status='train', pc_input_num=1024, aug=True):
        ...
        for _dir in npy_list:
            ...
            if '\\' in _dir:
                self.lbl_list.append(categorys.index(_dir.replace('\\', '/').split('/')[-3]))
            else:
...
class Modelnet40_data_AUG(data.Dataset):
    def __init__(self, pc_root, status='train', pc_input_num=1024, aug=True):
        super(Modelnet40_data_AUG, self).__init__()
    ...
    def __getitem__(self, idx):
        ...
        if self.aug:
            # base
            pc_base = rotation_point_cloud(pc)
            pc_base = jitter_point_cloud(pc_base)
            pc_base = np.expand_dims(pc_base.transpose(), axis=2)
            pc_base = torch.from_numpy(pc_base).type(torch.FloatTensor)
            
            # aug1
            pc_aug1 = pc_augment(pc)
            pc_aug1 = np.expand_dims(pc_aug1.transpose(), axis=2)
            pc_aug1 = torch.from_numpy(pc_aug1).type(torch.FloatTensor)
            
            # aug2
            pc_aug2 = pc_augment(pc)
            pc_aug2 = np.expand_dims(pc_aug2.transpose(), axis=2)
            pc_aug2 = torch.from_numpy(pc_aug2).type(torch.FloatTensor)
            return [pc_aug1, pc_aug2, pc_base], lbl, idx
        # print(pc.shape)
        # for test
        else:
            pc = np.expand_dims(pc.transpose(), axis=2)
...
class Shapenet_data_AUG(data.Dataset):
    def __init__(self, pc_root, status='train', pc_input_num=1024, aug=True, data_type='*.npy'):
        super(Shapenet_data_AUG, self).__init__()
    ...
    def __getitem__(self, idx):
        ...
        if self.aug:
            ...
            pc_base = rotation_point_cloud(pc)
            pc_base = jitter_point_cloud(pc_base)
            pc_base = shapenet_postprocess(pc_base, self.pc_input_num)
            
            # aug1
            pc_aug1 = pc_augment(pc)
            pc_aug1 = shapenet_postprocess(pc_aug1, self.pc_input_num)
            
            # aug2
            pc_aug2 = pc_augment(pc)
            pc_aug2 = shapenet_postprocess(pc_aug2, self.pc_input_num)
            return [pc_aug1, pc_aug2, pc_base], lbl, idx
            
        else:
            pc = shapenet_postprocess(pc, self.pc_input_num)
            return pc, lbl, idx
...
class Scannet_data_h5_AUG(data.Dataset):

    def __init__(self, pc_root, status='train', pc_input_num=1024, aug=True):
        super(Scannet_data_h5_AUG, self).__init__()
        self.num_points = pc_input_num
        self.status = status
        self.aug = aug
        # self.label_map = [2, 3, 4, 5, 6, 7, 9, 10, 14, 16]
    ...
    def __getitem__(self, idx):
        ...
        if self.aug:
            ...
            pc_aug2 = np.expand_dims(pc_aug2.transpose(), axis=2)
            pc_aug2 = torch.from_numpy(pc_aug2).type(torch.FloatTensor)
            return [pc_aug1, pc_aug2, pc_base], label, idx
        # print(pc.shape)
        # for test
        else:
            pc = np.expand_dims(pc.transpose(), axis=2)
...
if __name__ == "__main__":
    ...
    print (len(data))
    point, label, idx = data[0]
    print (point.shape, label, idx)
