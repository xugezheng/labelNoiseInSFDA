...
def main(args):
    ...
    criterion = criterion.to(device=args.device)
    ...
    optimizer_c = optim.Adam([{'params': model_c.parameters()}],
                             lr=LR * 2,
    ...
    for epoch in range(max_epoch):
        ...
        with torch.no_grad():
            ...
            # if pred_acc_t > best_source_test_acc:
                best_source_test_acc = pred_acc_s
                # best_source_test_acc = pred_acc_t
                best_netF = model_f.state_dict()
                best_netC = model_c.state_dict()

                torch.save(best_netF, args.output_dir+"/source_F.pt")
                torch.save(best_netC, args.output_dir+"/source_C.pt")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
            ...
            default="/storage/sfda_lln_extend/DATASOURCE",
        )
    parser.add_argument(
        "-model_root",
        help="model load and save root dir",
        default="/notebooks/sfda_lln_extend/Models/pointda_0827",
    )
    parser.add_argument(
        "-dset",
        type=str,
    ...
    args.output_dir = osp.join(args.model_root, f"seed{str(args.seed)}", args.dset, domain_2_name[args.source].upper())
    if not osp.exists(args.output_dir):
        try:
            os.makedirs(args.output_dir)
        except FileExistsError:
            print(f"Directory '{args.output_dir}' already exists. Skipping creation.")
