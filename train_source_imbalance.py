...
import vis_sourcefree
from utils_source_train import CrossEntropyLabelSmooth, ImageList_imb
...
def data_load(args):
    ...
    if args.source_balanced:
        ...
        dsets["source_tr"] = ImageList_imb(tr_txt, transform=image_train(), cfg=args, balance_sample=True)
    else:
        dsets["source_tr"] = ImageList_imb(tr_txt, transform=image_train(), cfg=args, balance_sample=False)
    dset_loaders["source_tr"] = DataLoader(dsets["source_tr"], batch_size=train_bs, shuffle=True,
                                           num_workers=args.worker, drop_last=False)
    # validation set
    dsets["source_te"] = ImageList_imb(te_txt, transform=image_test(), cfg=args, balance_sample=False)
    dset_loaders["source_te"] = DataLoader(dsets["source_te"], batch_size=train_bs, shuffle=True,
                                           num_workers=args.worker, drop_last=False)
    # test set
    dsets["test"] = ImageList_imb(txt_test, transform=image_test(), cfg=args, balance_sample=False)
...
def cal_acc(loader, netF=None, netB=None, netC=None, per_class_flag=False, visda_flag=False):
    ...
    mean_ent = torch.mean(Entropy(all_output)).cpu().data.item()  # average entropy of classification results
...
def train_source(args):
    ...
    if args.net[0:3] == 'res' or args.net[0:3] == 'vgg':
        if args.net[0:3] == 'res':
            netF = vis_sourcefree.ResBase(res_name=args.net).cuda()
        netB = vis_sourcefree.feat_bootleneck(type=args.classifier, feature_dim=netF.in_features,
                                       bottleneck_dim=args.bottleneck).cuda()   # classifier: bn
        netC = vis_sourcefree.feat_classifier(type=args.layer, class_num=args.class_num,
...
def test_target(args):
    ...
    if args.net[0:3] == 'res' or args.net[0:3] == 'vgg':
        ...
        netC = vis_sourcefree.feat_classifier(type=args.layer, class_num=args.class_num, bottleneck_dim=args.bottleneck).cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--root",
        metavar="DIR",
        help="root path of DATASOURCE, eg, /scratch/gxu86/sfda_lln_extend/DATASOURCE",
        default="~/scratch/sfda_lln_extend/DATASOURCE",
    )
    parser.add_argument(
        "--model_root",
        help="model load and save root dir",
        default="~/scratch/sfda_lln_extend/Models/shot_exp",
    )
    parser.add_argument(
        "--list_name",
        type=str,
        default="image_list",
        # choices=["image_list", "image_list_nrc", "image_list_nrc_partial"],
    )
    parser.add_argument(
        "--s_model", help="source model suffix name, e.g., new", default='imb_rs'
    )
    ...
    if args.dset == 'office-home-RSUT':
        ...
        args.s_dset_path = osp.join(folder, "VISDA-C",args.list_name, names[args.s] + '_RS.txt')
        args.test_dset_path = osp.join(folder, "VISDA-C",args.list_name, names[args.t] + '_UT.txt')
        args.s_val_path = osp.join(folder, "VISDA-C",args.list_name, names[args.s] + '_RS_BaVal.txt') # val set is balanced
    ...
    if args.s_model is not None:
        args.output_dir_src = osp.join(args.model_root, 'seed'+str(args.seed), 'VISDA-C', names[args.s][0].upper()+'_'+args.s_model)
    else:
        args.output_dir_src = osp.join(args.model_root, 'seed'+str(args.seed), 'VISDA-C', names[args.s][0].upper())
